...
function getSeasonNote(season) {
  if (season === 'summer') return 'It is summer — favor lighter, fresher meals (salads, grilled proteins, cold noodles). Avoid heavy casseroles.'
  if (season === 'winter') return 'It is winter — favor warming, hearty meals (soups, braises, roasts, pasta bakes).'
  if (season === 'fall') return 'It is fall — favor comforting meals with squash, root vegetables, and warming spices.'
  return 'It is spring — favor fresh, lighter meals with seasonal vegetables and bright flavors.'
}

function getAgeGroup(birthMonth, birthYear) {
  const now = new Date()
...
export async function POST(request) {
  try {
    ...
    var results = await Promise.all([
      ...
      sb.from('recipes').select('id, title, description, cuisine, total_time_mins, tags, dietary_flags, base_servings, is_favorite, in_rotation, rotation_frequency, last_planned_date, ingredients, instructions, average_rating, times_made').eq('profile_id', userId).order('is_favorite', { ascending: false }),
    ...
    var familyMembers = results[4].data || []

    // Season
    var season = getSeason()
    var seasonNote = getSeasonNote(season)

    // Family intelligence
    var childAgeGroups = familyMembers.map(function(m) { return getAgeGroup(m.birth_month, m.birth_year) })
    var hasToddlers = childAgeGroups.includes('toddler')
    var hasYoungChildren = childAgeGroups.includes('young-child') || hasToddlers
    var hasChildren = childAgeGroups.length > 0

    // Serving size — total family size including adults
    var familySize = (profile && profile.family_size) || 4

    // Pattern recognition — skip poorly rated recipes (avg < 2 with 2+ makes)
    vaultRecipes = vaultRecipes.filter(function(r) {
      if (r.times_made >= 2 && r.average_rating !== null && r.average_rating < 2) {
        console.log('[plan/generate] skipping low-rated: "' + r.title + '" avg=' + r.average_rating)
        return false
      }
      return true
    })
    ...
    // Build family context note
    var familyNote = 'Family of ' + familySize + '.'
    if (hasToddlers) familyNote += ' Has toddlers — avoid very spicy, complex, or chewy foods.'
    else if (hasYoungChildren) familyNote += ' Has young children — favor milder, kid-friendly options a few nights a week.'
    else if (hasChildren) familyNote += ' Has school-age children — kid-friendly meals welcome but not required every night.'
    ...
    return Response.json({
      ...
      stats: { vault: vaultRecipes.length, eligible: eligibleVault.length, system: systemRecipes.length, season: season, familySize: familySize }
    })
