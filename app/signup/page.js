...
// Reuse the same styles as login — imported via shared CSS vars
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .auth-root {
    ...
    --ember: #C05C30; --parchment: #F8F3EC; --sage-l: #8FA889;
  ...
  .auth-input {
    ...
    border-radius: 10px; padding: 13px 16px; color: var(--parchment);
  ...
  .auth-error { background: rgba(239,68,68,0.08); border: 1px solid rgba(239,68,68,0.2); border-radius: 8px; color: #EF4444; font-size: 0.82rem; padding: 10px 14px; margin-top: 12px; text-align: left; line-height: 1.5; }
    ...
    try {
      const { error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          // After email confirm, redirect to onboarding
      ...
      <div className="auth-root">
        ...
        <div className="auth-card">
          ...
          <p className="auth-sub">
            We&apos;ve sent a confirmation email to <strong style={{ color: 'var(--parchment)' }}>{email}</strong>.
            Click the link to verify your account and start your 14-day free trial.
          </p>
          <div className="auth-success">
            ✓ Once confirmed, you&apos;ll be taken through a quick setup to personalize Simply Sous for your family. Takes about 5 minutes.
          </div>
